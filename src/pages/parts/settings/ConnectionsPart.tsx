        ...
        <div className="my-3">
          ...
          <p className="max-w-[30rem] font-medium">
            <Trans i18nKey="settings.connections.workers.description">
              <MwLink to="https://kal-Stream.github.io/docs/proxy/deploy">
        ...
        <div className="my-3">
          ...
          <p className="max-w-[30rem] font-medium">
            <Trans i18nKey="settings.connections.server.description">
              <MwLink to="https://kal-Stream.github.io/docs/backend/deploy">
      ...
      <>
        <SettingsCard>
          <div className="flex justify-between items-center gap-4">
            <div className="my-3">
              <p className="text-white font-bold mb-3">
                {t("settings.connections.febbox.title")}
              </p>
              <p className="max-w-[30rem] font-medium">
                {t("settings.connections.febbox.description")}
              </p>
              <p className="max-w-[30rem] mt-2 text-sm text-type-secondary italic">
                {t("settings.connections.febbox.disclaimer")}
              </p>
            </div>
            <div>
              <Toggle
          ...
          {isFebboxVisible ? (
            <>
              ...
              <div className="my-3">
                <p className="max-w-[30rem] font-medium">
                  {t("settings.connections.febbox.stepsTitle")}
                  <br />
                  <div
                  ...
                  >
                    <span className="text-sm">
                      {showVideo ? t("settings.connections.febbox.hideVideo") : t("settings.connections.febbox.showVideo")}
                    </span>
                    {showVideo ? (
                  ...
                  <Trans i18nKey="settings.connections.febbox.step1">
                    1. Go to <MwLink url="https://febbox.com">febbox.com</MwLink> and log in with Google (use a fresh account!)
                  </Trans>
                  <br />
                  {t("settings.connections.febbox.step2")}
                  <br />
                  {t("settings.connections.febbox.step3")}
                  <br />
                  {t("settings.connections.febbox.step4")}
                  <button
                    type="button"
                    onClick={exampleModal.show}
                    className="text-type-link hover:text-type-linkHover"
                  >
                    {t("settings.connections.febbox.exampleButton")}
                  </button>
                  <br />
                  {t("settings.connections.febbox.step5")}
                </p>
              </div>

              <Divider marginClass="my-6 px-8 box-content -mx-8" />
              <p className="text-white font-bold mb-3">
                {t("settings.connections.febbox.token")}
              </p>
              ...
              {status === "error" && (
                <p className="text-type-danger mt-4">
                  {t("settings.connections.febbox.validationError")}
                </p>
              )}
              {status === "api_down" && (
                <p className="text-type-danger mt-4">
                  {t("settings.connections.febbox.apiDown")}
                </p>
              )}
              {status === "invalid_token" && (
                <p className="text-type-danger mt-4">
                  {t("settings.connections.febbox.invalidToken")}
                </p>
              )}
              {status === "success" &&
                quota &&
                  ...
                  return (
                    <>
                      <p className="text-sm text-green-500 mt-2">
                        {t("settings.connections.febbox.trafficLimit", { reset })}
                      </p>
                      <p className="max-w-[30rem] text-xs opacity-70 mt-2">
                        {t("settings.connections.febbox.trafficDisclaimer")}
                      </p>
                    </>
                  );
                })()}
              <div className="flex justify-between items-center gap-4 mt-6">
                <div className="my-3">
                  <p className="max-w-[32rem] font-medium">
                    {t("settings.connections.febbox.enableMp4")}
                  </p>
                </div>
                <div>
                  <Toggle
        ...
        <Modal id={exampleModal.id}>
          <ModalCard>
            <Heading2 className="!mt-0 !mb-4 !text-2xl">
              {t("settings.connections.febbox.exampleTitle")}
            </Heading2>
            <Paragraph className="!mt-1 !mb-6">
              {t("settings.connections.febbox.exampleDescription")}
            </Paragraph>
            ...
            <Paragraph className="!mt-1 !mb-6 text-type-danger">
              {t("settings.connections.febbox.fakeWarning")}
            </Paragraph>
            <div className="flex justify-end">
              <Button theme="secondary" onClick={exampleModal.hide}>
                {t("settings.connections.febbox.gotIt")}
              </Button>
            </div>
          </ModalCard>
        </Modal>
...
export function WyzieEdit() {
  ...
  return (
    <SettingsCard paddingClass="px-5 py-4">
      <p className="text-white font-bold mb-2">{t("settings.connections.wyzie.title")}</p>
      <p className="font-medium text-sm mb-3 text-type-secondary">
        <Trans i18nKey="settings.connections.wyzie.description">
          <MwLink url="https://store.wyzie.io/redeem" />
        </Trans>
      </p>
      <p className="text-white font-bold mb-2 text-sm">{t("settings.connections.wyzie.keyLabel")}</p>
      <AuthInputBox
        onChange={(newKey) => {
